...
function ihttp_request($url, $post = '', $extra = array(), $timeout = 60)
{
    ...
    if (function_exists('curl_init') && function_exists('curl_exec')) {
        ...
        if ($errno || empty($data)) {
            ...
            $temp = ihttp_response_parse($data);
            if (in_array($temp['code'], array('301', '302')) && $extra['CURLOPT_FOLLOWLOCATION']) {
                return ihttp_request($temp['headers']['Location'], $post, $extra, $timeout);
            } else {
...
function ihttp_email($to, $subject, $body, $global = false)
{
    ...
    if (empty($mailer)) {
        ...
        global $_W;
        $config = $GLOBALS['_W']['setting']['email'];
        if (!$global) {
            $row   = pdo_fetch("SELECT * FROM " . tablename('core_setting') . " WHERE `key`='email'");
            $email = @iunserializer($row['value']);
            if (!empty($email)) {
                $config = $email;
            }
        }
