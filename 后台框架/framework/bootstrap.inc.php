...
$urls['path']    = str_replace(array('/admin', '/web', '/wechat', '/app', '/payment', '/api'), '', $urls['path']);
...
$_W['setting']['upload']['video']['folder'] = "videos/" . date('Y/m/d/');
...
$entry      = empty($_GPC['entry']) ? 'app' : $_GPC['entry'];
$controller = $_GPC['c'];
$action     = $_GPC['a'];
$do         = $_GPC['do'];
$op         = $_GPC['op'];
...
load()->classs('status');
require IA_ROOT . '/framework/defines.inc.php';
