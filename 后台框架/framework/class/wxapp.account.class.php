...
class WxappAccount
{
    public function __construct($account = array())
    {
        ...
        if (empty($account)) {
            $this->wechat_mina_account = $_W['setting']['wechat_mina'];
        } else {
            $this->wechat_mina_account = $account;
        }
        if (empty($this->wechat_mina_account)) {
            $return = array(
                "status" => 0,
                "msg"    => '无效的appid及appsecret配置',
                "data"   => null,
            );
    ...
    public function getOauthInfo($code = '')
    {
        ...
        $url             = "https://api.weixin.qq.com/sns/jscode2session?appid={$this->wechat_mina_account['appid']}&secret={$this->wechat_mina_account['appsecret']}&js_code={$code}&grant_type=authorization_code";
        return $response = $this->requestApi($url);
    }

    public function pkcs7Encode($encrypt_data, $iv, $session_key)
    {
        ...
        if ($result['watermark']['appid'] != $this->wechat_mina_account['appid']) {
            return error(1, '解密失败');
        }
    ...
    {
        $cachekey = "accesstoken:{$this->wechat_mina_account['appid']}";
        $cache    = cache_load($cachekey);
        if (!empty($cache) && !empty($cache['token']) && $cache['expire'] > TIMESTAMP) {
            $this->wechat_mina_account['access_token'] = $cache;
            return $cache['token'];
        }

        if (empty($this->wechat_mina_account['appid']) || empty($this->wechat_mina_account['appsecret'])) {
            return error('-1', '未填写小程序的 appid 或 appsecret！');
        }

        $url      = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={$this->wechat_mina_account['appid']}&secret={$this->wechat_mina_account['appsecret']}";
        ...
        $this->wechat_mina_account['access_token'] = $record;
