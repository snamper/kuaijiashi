...
class DB
{
    ...
    public function getall($tablename, $params = array(), $fields = array(), $key = 'id', $after_where = '')
    {
        $select = '*';
        if (!empty($fields) && is_array($fields)) {
            $select = '`' . implode('`,`', $fields) . '`';
        ...
        $condition = $this->implode($params, 'AND');
        $sql       = "SELECT {$select} FROM " . tablename($tablename) . (!empty($condition['fields']) ? " WHERE {$condition['fields']}" : '');
        return $this->fetchall($sql . ' ' . $after_where, $condition['params'], $key);
    }
    ...
    public function debug($output = true, $append = array())
    {
        ...
        if ($output) {
            ...
            if (!empty($append['error'][1])) {
                ...
                message("SQL: <br/>{$append['sql']}<hr/>Params: <br/>{$params}<hr/>SQL Error: <br/>{$append['error'][2]}<hr/>Traces: <br/>{$ts}", '', 'sql');
            }
        }
        return $this->errors;
    }
