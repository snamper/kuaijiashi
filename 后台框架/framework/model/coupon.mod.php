...
{
    $coupon_template_all = pdo_fetch_many('coupon_template', array(), array(), 'id', 'ORDER BY `id` DESC');
...
function coupon_grant($uid, $coupon_template_id)
{
    ...
    if ($coupon_template['quota'] != 0) {
        $quota = coupon_quota($uid, $coupon_template_id);
        if ($quota >= $coupon_template['quota']) {
            return error(1, '每人限领' . $coupon_template['quota'] . '张');
        }
    }
...
function coupon_quota($uid, $coupon_template_id)
{
    $quota = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('coupon') . " WHERE `coupon_template_id` = :coupon_template_id AND `uid` = :uid", array(':coupon_template_id' => $coupon_template_id, ':uid' => $uid));
    return $quota;
}
...
{
    $sql    = 'UPDATE ' . tablename('coupon_template') . ' SET `quantity_issue` = `quantity_issue` + :quantity WHERE id=:id';
    $params = array(
...
{
    $sql    = 'UPDATE ' . tablename('coupon_template') . ' SET `quantity_used`=`quantity_used`+:quantity WHERE id=:id';
    $params = array(
        ':id'       => $coupon_template_id,
        ':quantity' => $quantity,
    );
...
function coupon_handle($order, $coupon_id)
{
    ...
    if ($coupon['is_at_least'] == YES && $order['rentfee'] < $coupon['at_least']) {
    ...
    $cash     = currency_format($coupon['value']);
    $activity = array(
        'order_id'    => $order['id'],
        'activity'    => 'coupon',
    ...
    pdo_update('drive_coach_order', array('discount_fee' => $order['discount_fee'], 'payment' => $order['payment']), array('id' => $order['id']));

    return true;
}

function order_discount_fee($order)
{
    $discount_fee = 0;

    $activities = pdo_fetch_many('order_activity', array('order_id' => $order['id']));
    if ($activities) {
        $cach = 0;

        foreach ($activities as $activity) {
            if ($activity['code'] == 'cash') {
                $cach += $activity['value'];
            }
        }
    ...
    return currency_format($discount_fee);
}

function order_payment($order)
{
    return currency_format($order['total'] - $order['discount_fee']);
}
