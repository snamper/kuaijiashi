...
<ul class="nav nav-tabs">
	<li <?php  if($_GPC['do'] == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo url('system/banner', array('do'=>'display', 'rid' => $rid))?>">幻灯片</a></li>
	<li <?php  if($_GPC['do'] == 'post') { ?>class="active"<?php  } ?>><a href="<?php  echo url('system/banner', array('rid' => $rid, 'do' => 'post' ))?>">添加幻灯片</a></li>
</ul>
				 ...
				 <div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						<label class='radio-inline'>
							<input type='radio' name='status' value='1' <?php  if($banner['status']==1) { ?>checked<?php  } ?> /> 是
						</label>
						<label class='radio-inline'>
							<input type='radio' name='status' value='0' <?php  if($banner['status']==0) { ?>checked<?php  } ?> /> 否
						</label>
					</div>
				</div>
			</div>
					...
					<tbody>
						<?php  if(is_array($list)) { foreach($list as $banner) { ?>
							<tr>
								...
								<td><?php  if($banner['status']) { ?>显示<?php  } else { ?>隐藏<?php  } ?></td>
								<td style="text-align:left;">
									<a href="<?php  echo url('system/banner', array('do' => 'post', 'id' => $banner['id'], 'rid' => $rid))?>" data-toggle="tooltip" data-placement="top"  class="btn btn-default btn-sm manage"><i class="fa fa-edit"></i>修改</a>
									<a onclick="return confirm('确认要删除吗？删除将不能恢复！');" href="<?php  echo url('system/banner', array('do' => 'delete', 'id' => $banner['id'],'rid' => $rid))?>" data-toggle="tooltip" data-placement="top"  class="btn btn-default btn-sm manage"><i class="fa fa-del"></i>删除</a> 
								</td>
							</tr>
						<?php  } } ?>
