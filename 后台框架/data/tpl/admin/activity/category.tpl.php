...
<ul class="nav nav-tabs">
	<li <?php  if($do == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo url('activity/category', array('do' => 'display'))?>">管理分类</a></li>
	<li <?php  if($do == 'post' && empty($id)) { ?>class="active"<?php  } ?>><a href="<?php  echo url('activity/category', array('do' => 'post'))?>">添加分类</a></li>
...
<div class="main">
    <div class="category">
			...
			<?php  if(is_array($category)) { foreach($category as $row) { ?>
				<tr>
					...
					<td>
						...
						<?php  if(empty($row['parentid'])) { ?>
							<a href="<?php  echo url('activity/category', array('parentid' => $row['id'], 'do' => 'post'))?>" class="btn btn-default btn-success"><i class="fa fa-plus-circle"></i> 添加子分类</a>
						<?php  } ?>
						</div>
					</td>
                    <td><div class="type-parent"><?php  echo $row['description'];?></div></td>
					<td><a href="<?php  echo url('activity/category', array('do' => 'post', 'id' => $row['id']))?>" class="btn btn-default"><i class="fa fa-edit"></i> 编辑</a>&nbsp;&nbsp;<a href="<?php  echo url('activity/category', array('do' => 'delete', 'id' => $row['id']))?>" class="btn btn-default" onclick="return confirm('确认删除此分类吗？');return false;"><i class="fa fa-remove"></i> 删除</a></td>
				</tr>
				<?php  if(is_array($children[$row['id']])) { foreach($children[$row['id']] as $item) { ?>
					<tr>
						...
						<td><a href="<?php  echo url('activity/category', array('do' => 'post', 'id' => $item['id'], 'parentid' => $item['parentid']))?>" class="btn btn-default" title="编辑"><i class="fa fa-edit"></i> 编辑</a>&nbsp;&nbsp;<a href="<?php  echo url('activity/category', array('do' => 'delete', 'id' => $item['id']))?>" class="btn btn-default" onclick="return confirm('确认删除此分类吗？');return false;" title="删除"><i class="fa fa-remove"></i> 删除</a></td>
					</tr>
				<?php  } } ?>
			<?php  } } ?>
			</tbody>
		...
		<div class="form-group col-sm-12">
						<a href="<?php  echo url('activity/category', array('do' => 'post'))?>" class="btn btn-default btn-primary"><i class="fa fa-plus-circle"></i> 添加新分类</a>
