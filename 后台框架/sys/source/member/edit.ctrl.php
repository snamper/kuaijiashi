...
if ($do == 'display') {
    $uid         = $_GPC['uid'];
    $member      = mc_member($uid);
    $memberRoles = $_W['setting']['memberroles'];
    $return      = array(
        'userInfo'    => $member,
...
if ($do == 'post') {
    ...
    if (!empty($_GPC['userInfo'])) {
        ...
        if (($email_effective == 1 && empty($_GPC['email']))) {
            unset($_GPC['userInfo']['email']);
        }
        if (!empty($_GPC['userInfo']['profile'])) {
            mc_update($uid, $_GPC['userInfo']['profile']);
            unset($_GPC['userInfo']['profile']);
        }
        mc_update($uid, $_GPC['userInfo']);
