...
if ($do == 'getList') {
    ...
    $size     = 10;
    $page     = $_GPC['page'];
    $sqlTotal = pdo_sql_select_count_from('drive_bidding') . $condition;
    $sqlData  = pdo_sql_select_all_from('drive_bidding') . $condition . ' ORDER BY `createtime` DESC ';
...
if ($do == 'delete') {
    $id = $_GPC['id'];
    if (pdo_delete('drive_bidding', array('id' => $id))) {
